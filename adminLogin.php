...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) && !empty($password)) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $stmt = $conn->prepare("SELECT ad_password FROM admins WHERE ad_email = ?");
            ...
            if ($stmt->num_rows > 0) {
                ...
                if (password_verify($password, $stored_password)) {
                    ...
                    header("Location: homePage2.php"); // Redirect to admin dashboard
                    exit();
                } else {
...
<body>
  ...
  <div class="login-container">
    ...
    <form method="POST" action="">
      ...
      <button type="submit" class="btn login-btn p">Log In</button>
    </form>

    <!-- Admin Button -->
    <form method="POST" action="./Login.php"><button class="btn admin-btn">Log In As User</button></form>
	<!-- Sign Up Link -->
    <p class="sign-up">Don’t You Have Any Account? <a href="./AdminSignUp.php">Sign Up</a></p>
